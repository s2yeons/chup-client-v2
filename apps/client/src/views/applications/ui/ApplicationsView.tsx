...
const ApplicationsView = () => {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <p className="text-sm font-semibold text-primary">지원 현황</p>
        <h1 className="mt-1 text-3xl font-bold">지원 여정을 확인하세요</h1>
        <p className="mt-2 text-muted-foreground">제출한 지원서와 전형 결과를 한눈에 확인할 수 있어요.</p>
      </div>
      <div className="grid gap-4 sm:grid-cols-3">
        <StatCard label="전체 지원" value={`${myApplications.length}건`} note="누적 지원 수" icon={Send} />
        <StatCard
          label="검토 중"
          ...
          {myApplications.map((application) => (
            ...
            >
              <div className="flex items-center gap-3">
                <div className="flex size-11 items-center justify-center rounded-xl bg-secondary font-bold text-primary">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">{application.date} 지원</p>
                </div>
              </div>
